...
from frappe_tsx.frappe_tsx.utils import create_file


def create_type_definition_file(doc, method=None):
	common_site_config = frappe.get_conf()

	frappe_tsx_pause_generation = common_site_config.get("frappe_tsx_pause_generation", 0)

	if frappe_tsx_pause_generation:
		print("Frappe TSX is paused")
		return
	...
	if is_developer_mode_enabled() and is_valid_doctype(doctype):
		...
		for type_setting in type_generation_settings:
			if app_name == type_setting.app_name:
				...
				generate_type_definition_file(doctype, module_path, generate_child_tables=False)
...
def generate_type_definition_file(doctype, module_path, generate_child_tables=False):
	...
	type_file_content = generate_type_definition_content(doctype, module_path, generate_child_tables)
...
def get_imports_for_table_fields(field, doctype, module_path, generate_child_tables):
	if field.fieldtype == "Table" or field.fieldtype == "Table MultiSelect":
		...
		if doctype_module_name == table_module_name:
			table_file_path: Path = module_path / (table_doc.name.replace(" ", "") + ".ts")
			...
			table_file_path: Path = table_module_path / (table_doc.name.replace(" ", "") + ".ts")
...
def before_migrate():
	...
	subprocess.run(["bench", "config", "set-common-config", "-c", "frappe_tsx_pause_generation", "1"])


def after_migrate():
	# print("After migrate")
	subprocess.run(["bench", "config", "set-common-config", "-c", "frappe_tsx_pause_generation", "0"])
...
def generate_types_for_doctype(doctype, app_name, generate_child_tables=False, custom_fields=False):
	try:
		...
		doc = frappe.get_meta(doctype) if custom_fields else frappe.get_doc("DocType", doctype)
		...
		if is_developer_mode_enabled() and is_valid_doctype(doc):
			...
			for type_setting in type_generation_settings:
				if app_name == type_setting.app_name:
					...
					generate_type_definition_file(doc, module_path, generate_child_tables)
